...
- **Markdown or richer structured output.** HTML and XHTML tags (`<h1>`–`<h6>`, `<table>`, `<strong>`, `<a>`, inline XBRL) carry semantic information that plain text discards: section hierarchy, table row/column structure, deliberate emphasis, cross-references between filings and exhibits. V0 classification asks content questions ("is this an executive departure?") that survive the lossy reduction; later features ("find filings where a disclosure is buried in a footnote", "compare risk factors across peers", "extract a table of insider trades") will need more structure. The path forward is adding a `markdown` field alongside `text` on `FilingDocument` when the first feature needs it, not retrofitting the whole pipeline.
