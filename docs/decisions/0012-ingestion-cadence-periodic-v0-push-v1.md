...
- `TimeoutStartSec=12m` bounds a hung run; `systemd` kills it, marks the unit failed, and the next tick fires after the configured gap. Failures are observable via `systemctl is-failed` and `journalctl -u`.
