...
1. **No resource isolation on the classifier.** The ingest/reconciler ticks run as systemd one-shot services with no cgroup memory limit, so a single tick can consume all host RAM. With swap present, memory pressure would manifest as swap/I/O thrash — a livelock — rather than a contained, fail-fast kill. (The exact mechanism of the wedge was never confirmed — the kernel log showed no OOM-killer and a later drain held flat — but this is the failure mode the unbounded setup enables.) The box ran for weeks on 8-K volume; adding 6-K classification was enough new load to expose this.
