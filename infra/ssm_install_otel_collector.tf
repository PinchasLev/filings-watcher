    ...
    mainSteps = [{
      ...
      inputs = {
        runCommand = [
          ...
          "# OpenTelemetry Collector config — verification-only.",
          "#",
          "# Receivers: OTLP on localhost only (apps share the host with the",
          "# Collector; no need to expose OTLP off-host).",
          "#",
          "# Exporters: debug (prints arrivals to stdout, captured by journald)",
          "# and prometheus (exposes incoming metrics as Prometheus exposition",
          "# format on :8889 for curl-based verification). No backend exporter",
          "# in this iteration; PR 2 adds journald receiver + first backend.",
          "",
          "receivers:",
          "  otlp:",
          "    protocols:",
