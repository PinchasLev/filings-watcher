...
def _fresh_db() -> Engine:
    ...
    ensure_taxonomy_snapshot(engine)  # cut the current version (v1)
    return engine


def test_leaves_for_version_returns_the_snapshot_leaves() -> None:
    engine = _fresh_db()
    leaves = set(leaves_for_version(engine, "v1"))
    assert leaves == {e.value for e in EventType}  # v1 == the full in-code taxonomy today


def test_baseline_leaves_are_eventtypes_in_declaration_order() -> None:
    engine = _fresh_db()
    baseline = _baseline_leaves(engine, "v1")
